...
void LaunchApp(const std::string& path,bool isWeb=false){
    ...
    std::string dir=path; auto p=dir.find_last_of("\\/"); if(p!=std::string::npos)dir=dir.substr(0,p);
    SHELLEXECUTEINFOA s={sizeof(s)};s.fMask=SEE_MASK_NOCLOSEPROCESS;s.lpVerb="open";
    s.lpFile=path.c_str();s.lpDirectory=dir.c_str();s.nShow=SW_SHOWNORMAL;
...
StartupChoice ShowLaunchDialog(){
    ...
    D2D().Init(hw,W,H);
    InputAdapter input; int sel=0; float an=0; StartupChoice res=StartupChoice::NONE;
    MSG msg; bool running2=true;
    while(running2){
        while(PeekMessage(&msg,nullptr,0,0,PM_REMOVE)){TranslateMessage(&msg);DispatchMessage(&msg);if(msg.message==WM_QUIT){res=StartupChoice::NONE;running2=false;}}
        if(!running2)break;
...
bool ShowExitShellConfirmation(){
    ...
    while(!done){
        ...
        if(input.IsMoveLeft())sel=0; if(input.IsMoveRight())sel=1;
    ...
    D2D().Shutdown(); DestroyWindow(hw); return res;
}
...
int WINAPI WinMain(HINSTANCE,HINSTANCE,LPSTR,int){
    ...
    if(g_app.isShellMode){
        ...
        MessageBoxA(nullptr,"A: Dialog returned","DBG",MB_OK);
        switch(choice){
        ...
        MessageBoxA(nullptr,"B: Switch done","DBG",MB_OK);
    }

    MessageBoxA(nullptr,"C: Past dialog block","DBG",MB_OK);
    int sw=GetSystemMetrics(SM_CXSCREEN),sh=GetSystemMetrics(SM_CYSCREEN);
    if(sw<=0)sw=1920; if(sh<=0)sh=1080;
    MessageBoxA(nullptr,"D: LoadProfile starting","DBG",MB_OK);
    LoadProfile();
    MessageBoxA(nullptr,"E: LoadLibrary starting","DBG",MB_OK);
    LoadLibraryFromDisk(); LoadCustomAppsFromProfile();
    InitDefaultApps(); InitPlatformConnections();
    MessageBoxA(nullptr,"F: CreateMainWindow starting","DBG",MB_OK);
    HWND hw=CreateMainWindow(sw,sh,g_app.isShellMode);
    MessageBoxA(nullptr,"G: D2D Init starting","DBG",MB_OK);
    bool d2dOk=D2D().Init(hw,sw,sh);
    { char buf[64]; sprintf(buf,"H: D2D Init = %s",d2dOk?"OK":"FAILED"); MessageBoxA(nullptr,buf,"DBG",MB_OK); }
    g_app.mainWindow=hw;
    ...
    MessageBoxA(nullptr,"I: InitSkins starting","DBG",MB_OK);
    InitSkins();
    MessageBoxA(nullptr,"J: g_audio.Init starting","DBG",MB_OK);
    g_audio.Init();
    MessageBoxA(nullptr,"K: PlayStartup + LoadBackground","DBG",MB_OK);
    g_audio.PlayStartup();
    LoadBackground(g_app.bgPath);
    MessageBoxA(nullptr,"L: LoadCustomAppIcons","DBG",MB_OK);
    LoadCustomAppIcons(); LoadHubSliderTextures();
    MessageBoxA(nullptr,"M: RefreshLibrary starting","DBG",MB_OK);
    ...
    RefreshLibrary();
    MessageBoxA(nullptr,"N: LoadGamePosters starting","DBG",MB_OK);
    LoadGamePosters();
    MessageBoxA(nullptr,"O: GetSteamProfile starting","DBG",MB_OK);
    g_app.steamProfile=GetSteamProfile();
    MessageBoxA(nullptr,"P: GetRealSteamFriends starting","DBG",MB_OK);
    g_app.steamFriends=GetRealSteamFriends();
    MessageBoxA(nullptr,"Q: LoadSteamAvatar starting","DBG",MB_OK);
    LoadSteamAvatar();
    MessageBoxA(nullptr,"R: Entering main loop","DBG",MB_OK);
